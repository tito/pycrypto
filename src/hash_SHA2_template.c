...
/* compress one block  */
static void PCFN(sha_compress)(hash_state * hs)
{
 ...
 */
static int PCFN(add_length)(hash_state *hs, sha2_word_t inc) {
...
/* init the SHA state */
static void PCFN(sha_init)(hash_state * hs)
{
    int i;
...
static void PCFN(sha_process)(hash_state * hs, unsigned char *buf, int len)
{
    while (len--) {
        /* copy byte */
...
static void PCFN(sha_done)(hash_state * hs, unsigned char *hash)
{
    int i;
...
// Done
static void PCFN(hash_init) (hash_state *ptr)
{
	PCFN(sha_init)(ptr);
}
...
// Done
static void
PCFN(hash_update) (hash_state *self, const U8 *buf, int len)
{
	PCFN(sha_process)(self,(unsigned char *)buf, len);
}

// Done
static void
PCFN(hash_copy)(hash_state *src, hash_state *dest)
{
...
// Done
static PyObject *
PCFN(hash_digest) (const hash_state *self)
{
	unsigned char digest[DIGEST_SIZE];
	hash_state temp;

	PCFN(hash_copy)((hash_state*)self,&temp);
	PCFN(sha_done)(&temp,digest);
